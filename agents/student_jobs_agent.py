...
class StudentJobsModel:
    ...
    async def execute_function(self, function_call):
        ...
        if function_name in function_mapping:
            ...
            self.logger.info(f"@student_jobs_agent.py Jobs : Function loop response : {func_response}")
            
            if func_response:
                return func_response
            else:
                self.logger.error(f"@student_jobs_agent.py Error extracting text from response: {e}")
    ...
    def _initialize_model(self):
        if not self.model:
            return self.logger.error(f"@student_jobs_agent.py Model not initialized at ActionFunction")
    ...
    async def determine_action(self,instruction_to_agent:str,special_instructions:str) -> str:
        ...
        try:
            ...
            self.logger.info(f"@student_jobs_agent.py Internal response @ Jobs Model : {response}")
            
            for part in response.parts:
                if hasattr(part, 'function_call') and part.function_call:
                    
                    final_response = await self.execute_function(part.function_call)
                    self.firestore.update_message("jobs_agent_message", f"Function called {part.function_call}  Function Response {final_response} ")
                    ...
                    if not text.startswith("This query") and not "can be answered directly" in text:
                        ...
                        self.logger.info(f"@student_jobs_agent.py text response : {final_response}")
        
        # Return only the final message
            return final_response if final_response else "Jobs agent fell off! Error 404"
            
        except Exception as e:
            self.logger.error(f"@student_jobs_agent.py Internal Error @ Jobs Model : {str(e)}")
