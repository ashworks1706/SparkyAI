...
class NewsMediaModel:
    ...
    def __init__(self,middleware,genai,app_config,logger,agent_tools):
        ...
        self.model = genai.GenerativeModel(
            ...
            {self.app_config.get_news_media_agent_instruction()}
            """,
            tools=[
                genai.protos.Tool(
    ...
    async def execute_function(self, function_call):
        ...
        if function_name in function_mapping:
            ...
            self.logger.info(f"News : Function loop response : {func_response}")
    ...
    async def determine_action(self,instruction_to_agent:str,special_instructions:str) -> str:
        ...
        try:
            ...
            prompt = f"""
                ...
                {self.app_config.get_news_media_agent_prompt()}
                
                """

            response = await self.chat.send_message_async(prompt)
            self.logger.info(f"Internal response @ News Model : {response}")
            
            for part in response.parts:
                if hasattr(part, 'function_call') and part.function_call:
                    
                    final_response = await self.execute_function(part.function_call)
                    self.middleware.update_message("news_media_agent_message", f"Function called {part.function_call}  Function Response {final_response} ")
                elif hasattr(part, 'text') and part.text.strip():
                    text = part.text.strip()
                    self.middleware.update_message("news_media_agent_message", f"Text Response : {text}")
