...
class Shuttle_Status_Model:
    ...
    def __init__(self,middleware,genai,app_config,logger,shuttle_status_agent_tools):
        self.logger = logger
    ...
    async def execute_function(self, function_call): 
        ...
        if function_name in function_mapping:
            ...
            self.logger.info(f"@shuttle_status_agent.py Live Status : Function loop response : {func_response}")
            
            if func_response:
                return func_response
            else:
                self.logger.error(f"@shuttle_status_agent.py Error extracting text from response: {e}")
    ...
    def _initialize_model(self):
        if not self.model:
            return self.logger.error(f"@shuttle_status_agent.py @shuttle_status_agent.py Model not initialized at ActionFunction")
    ...
    async def determine_action(self,instruction_to_agent:str,special_instructions:str) -> str:
        ...
        try:
            ...
            self.logger.info(f"@shuttle_status_agent.py Internal response @ Live Status Model : {response}")
            
            for part in response.parts:
                if hasattr(part, 'function_call') and part.function_call:
                    
                    final_response = await self.execute_function(part.function_call)
                    self.middleware.update_message("shuttle_status_agent_message", f"Function called {part.function_call}  Function Response {final_response} ")
                elif hasattr(part, 'text') and part.text.strip():
                    text = part.text.strip()
                    self.middleware.update_message("shuttle_status_agent_message", f"Text Response : {text}")
                    if not text.startswith("This query") and not "can be answered directly" in text:
                        final_response = text.strip()
                        self.logger.info(f"@shuttle_status_agent.py text response : {final_response}")
        
        # Return only the final message
            return final_response if final_response else "Live Status agent fell off! Error 404"
            
        except Exception as e:
            self.logger.error(f"@shuttle_status_agent.py Internal Error @ Live Status Model : {str(e)}")
