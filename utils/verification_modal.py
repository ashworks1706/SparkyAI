...
class VerificationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.stored_otp:  # First submission - email only
            if self.validate_asu_email(self.email.value):
                ...
                logger.info(f"@verification_modal.py Generated OTP: %s", self.stored_otp)
                ...
                async def button_callback(button_interaction):
                    logger.info(f"@verification_modal.py OTP button clicked for email: %s", self.email.value)
                ...
                logger.info(f"@verification_modal.py OTP sent to email: %s", self.email.value)
            else:
                await interaction.response.send_message("Invalid ASU email. Please try again.", ephemeral=True)
                logger.warning(f"@verification_modal.py Invalid ASU email entered: %s", self.email.value)

    def validate_asu_email(self, email):
        is_valid = re.match(r'^[a-zA-Z0-9._%+-]+@asu\.edu$', email) is not None
        logger.info(f"@verification_modal.py Email validation for %s: %s", email, is_valid)
        return is_valid

    def generate_otp(self):
        otp = ''.join(str(random.randint(0, 9)) for _ in range(6))
        logger.info(f"@verification_modal.py Generated OTP: %s", otp)
        return otp
    ...
    def send_otp_email(self, email, otp):
        ...
        try:
            ...
            logger.info(f"@verification_modal.py OTP email sent to %s", email)
        except Exception as e:
            logger.error(f"@verification_modal.py Failed to send OTP email to %s: %s", email, str(e))