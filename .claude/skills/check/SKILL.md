...
Run from `apps/backend`, in order, stopping at the first failure:
...
If Python packages changed (`services/ingest`, `models`): `uvx ruff check . && uvx ruff format --check . && uv run pytest -q` in that package. If `apps/web` changed: `npm run lint && npm run build`.
