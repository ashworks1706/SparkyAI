...
3. Create `crates/<name>/Cargo.toml` (library) or `apps/<name>/Cargo.toml` with a `[[bin]]` (process):
   ...
   - libraries depend on `sparky-harness` only; apps depend on `sparky-runtime` plus whatever crates they wire
