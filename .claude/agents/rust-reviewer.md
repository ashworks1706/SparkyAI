...
1. Architecture invariants: module dependency direction inside `engine` (`runtime::harness` imports nothing; `runtime::model`, `runtime::tools`, `clients` import only `runtime::harness`; never each other), the engine opens no database connection, no global state, traits for replaceable deps, request path makes no external fetches, model output never stored as evidence, write-side tools gated by `Policy`.
